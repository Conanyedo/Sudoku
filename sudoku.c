...
/*   Updated: 2021/06/26 11:31:10 by ybouddou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	fixed_fields(t_sudo *sudo)
{
	...
	while (++y < 9)
	{
		...
		while (++x < 9)
		{
			if (sudo->solved[y][x] != 0)
			{
				sudo->fixed[y][x] = 1;
				sudo->row[y] = (1 << (sudo->solved[y][x] - 1)) | sudo->row[y];
				sudo->col[x] = (1 << (sudo->solved[y][x] - 1)) | sudo->col[x];
				cell = which_cell(x, y);
				sudo->cell[cell] = (1 << (sudo->solved[y][x] - 1)) | sudo->cell[cell];
			}
		}
	}
}
...
void	user_input(t_sudo *sudo)
{
	...
	while (1)
	{
		...
		{
			y = ft_atoi(sudo->input + 3);
			x = ft_atoi(sudo->input + 5);
			n = ft_atoi(sudo->input + 7);
			if (!sudo->fixed[y][x])
				sudo->grid[y][x] = n;
			else
		...
		if (if_solved(sudo))
		{
			printf("\e[0;32mCongratulations!!!\e[0m\n");
			exit (0);
		}
	}
}
